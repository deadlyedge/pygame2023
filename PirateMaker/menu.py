...
class Menu:
    ...
    def create_buttons(self):
        ...
        self.enemy_button_rect = generic_button_rect.move(
            self.rect.width / 2,
            self.rect.height / 2).inflate(-button_margin, -button_margin)
        ...
        for sprite in self.buttons:
            if sprite.rect.collidepoint(mouse_pos):
                if mouse_button[1]:  # middle mouse click
                    sprite.main_active = not sprite.main_active if sprite.items[
                        'alt'] else True
...
class Button(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        rect = surf.get_rect(center=(self.rect.width / 2,
                                     self.rect.height / 2))
