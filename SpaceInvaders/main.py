...
class Game:
    ...
    def __init__(self):
        ...
        player_sprite = Player((screen_width / 2, screen_height), screen_width,
                               5)
        self.player = pygame.sprite.GroupSingle(player_sprite)  # type: ignore
        ...
        self.explosion_sound = pygame.mixer.Sound(
            'SpaceInvaders/audio/explosion.wav')
        self.explosion_sound.set_volume(0.3)

    def create_obstacle(self, x_start: int, y_start: int, offset_x):
        for row_index, row in enumerate(self.shape):
            for col_index, col in enumerate(row):
                if col == 'x':
                    x = x_start + col_index * self.block_size + offset_x
                    y = y_start + row_index * self.block_size
                    block = obstacle.Block(self.block_size, (241, 79, 80), x,
                                           y)
...
class CRT:
    ...
    def __init__(self) -> None:
        self.tv = pygame.image.load(
            'SpaceInvaders/graphics/tv.png').convert_alpha()
